...
install_pkg(){
  ...
  case "$MODE" in
    flakes)
      # add to home-manager or just nix profile
      if command -v home-manager &>/dev/null; then
        log_op "Tip: add '${name}' to your home.nix packages list and run 'home-manager switch'"
      ...
      ;;
    configuration)
      log_op "Tip: add pkgs.${name} to environment.systemPackages in configuration.nix"
      log_op "Then run: sudo nixos-rebuild switch"
      log_op "Installing temporarily with nix-env..."
  ...
  case "$MODE" in
    flakes)
      ...
      if nix profile list 2>/dev/null | grep -q "$name"; then
        nix profile remove "$(nix profile list 2>/dev/null | grep "$name" | awk '{print $1}')" 2>&1
      else
        nix-env -e "$name" 2>&1
      fi
      ;;
    configuration)
      log_op "Tip: remove pkgs.${name} from environment.systemPackages"
      log_op "Then run: sudo nixos-rebuild switch"
      log_op "Removing from nix-env (if present)..."
      nix-env -e "$name" 2>&1
      ;;
    imperative)
      nix-env -e "$name" 2>&1
...
load_installed(){
  ...
  (
    mapfile -t lines < <(list_installed_with_version 2>/dev/null)
    printf '%s\n' "${lines[@]}" > "$RESULT_FILE"
  ) &
  BG_PID=$!
}
...
finish_load_installed(){
  ...
  log_op "✓ Loaded ${#INST_LIST[@]} installed packages"
  STATUS_MSG="Mode: $(mode_label)  ·  ${#INST_LIST[@]} installed"
