...
start_remove(){
  ...
  log "… Removing '${name}'…"
  _run_pkg_op "remove" "$name"
}

# Write a self-contained python3 script to disk and run it as a separate process
# so it doesn't depend on bash function inheritance
_run_pkg_op(){
  local action="$1" pkg="$2"
  local helper="$TMPD/pkgop.py"
  local res="$RESULT" cf="$CONF_FILE" mode="$MODE"
...
action = sys.argv[1]   # add | remove
pkg    = sys.argv[2]
mode   = sys.argv[3]
cf     = sys.argv[4]   # config file path

def edit_nix(action, pkg, path):
    content = open(path).read()
    ...
    for pat, uw in pats:
        ...
        if m: use_with = uw; break
    if not m:
        print(f"ERROR: no packages list found in {path}", file=sys.stderr)
        sys.exit(1)
    ...
    if action == 'add':
        ...
        for line in inner.splitlines():
            ...
            if s and not s.startswith('#'):
                indent = line[:len(line)-len(s)]; break
        new_inner = inner.rstrip('\n') + f"\n{indent}{entry}\n"
        new_content = content[:m.start()] + m.group(1) + new_inner + m.group(3) + content[m.end():]
        # backup
        open(path + '.nixpkg.bak', 'w').write(content)
        open(path, 'w').write(new_content)
        print(f"Added '{entry}' to {path}")

    elif action == 'remove':
        new_lines = []; removed = 0
        block = m.group(1) + inner + m.group(3)
        before = content[:m.start()]
        after  = content[m.end():]
        for line in block.splitlines(keepends=True):
            if (re.search(rf'^\s*{re.escape(pkg)}\s*(#.*)?$', line) or
                re.search(rf'\bpkgs\.{re.escape(pkg)}\b', line)):
                removed += 1
            else:
                new_lines.append(line)
        if removed == 0:
            print(f"ERROR: '{pkg}' not found in packages list in {path}", file=sys.stderr)
            sys.exit(1)
        open(path + '.nixpkg.bak', 'w').write(content)
        open(path, 'w').write(before + ''.join(new_lines) + after)
        print(f"Removed '{pkg}' from {path} ({removed} line(s))")

def do_rebuild(cf):
    use_hm = ('home.packages' in open(cf).read() if os.path.isfile(cf) else False)
    if use_hm and subprocess.run(['which','home-manager'],capture_output=True).returncode == 0:
        print("Running: home-manager switch")
        r = subprocess.run(['home-manager','switch'], capture_output=True, text=True)
    else:
        print("Running: sudo nixos-rebuild switch")
        r = subprocess.run(['sudo','nixos-rebuild','switch'], capture_output=True, text=True)
...
rc = 0
if mode in ('configuration', 'flakes'):
    if not cf or not os.path.isfile(cf):
        print(f"ERROR: config file not found: {cf}", file=sys.stderr)
        sys.exit(1)
    edit_nix(action, pkg, cf)  # exits on error
    rc = do_rebuild(cf)
elif mode == 'imperative':
    if action == 'add':
        r = subprocess.run(['nix-env','-iA',f'nixpkgs.{pkg}'], capture_output=True, text=True)
    else:
        r = subprocess.run(['nix-env','-e',pkg], capture_output=True, text=True)
...
PYEOF
  ...
  python3 "$helper" "$action" "$pkg" "$mode" "$cf" > "$res" 2>&1 &
  BG_PID=$!
}
