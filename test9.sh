...
# Add package to nix config — auto-detects style (with pkgs; or pkgs.)
conf_add_package(){
  local file="$1" pkg="$2"
...
with open(path) as f:
    content = f.read()
with_pkgs = re.compile(
    r'((?:environment\.systemPackages|home\.packages)\s*=\s*with\s+pkgs\s*;\s*\[)(.*?)(\];)',
    re.DOTALL)
bare = re.compile(
    r'((?:environment\.systemPackages|home\.packages)\s*=\s*\[)(.*?)(\];)',
    re.DOTALL)
m = with_pkgs.search(content)
use_with = m is not None
if not m:
    m = bare.search(content)
if not m:
    print(f"ERROR: packages list not found in {path}", file=sys.stderr); sys.exit(1)
inner = m.group(2)
entry = pkg if use_with else f"pkgs.{pkg}"
if re.search(rf'\b{re.escape(entry)}\b', inner):
    print(f"{entry} already present"); sys.exit(0)
indent = "  "
for line in inner.splitlines():
    stripped = line.lstrip()
    if stripped and not stripped.startswith('#'):
        indent = line[:len(line)-len(stripped)]; break
...
# Remove package from nix config — handles both styles (bare name or pkgs.name)
conf_remove_package(){
  local file="$1" pkg="$2"
  cp "$file" "${file}.nixpkg.bak"
  python3 - "$file" "$pkg" <<'PYEOF'
...
for line in lines:
    ...
    out.append(line)
if removed == 0:
    print(f"ERROR: {pkg} not found", file=sys.stderr); sys.exit(1)
with open(path, 'w') as f:
    f.writelines(out)
...
BG_PID=""
BG_OP=""   # "load" | "search" | "install" | "remove"

bg_start(){
  LOADING=1
  LOADING_MSG="$1"
  BG_OP="$2"
  rm -f "$RESULT_FILE" "${TMPDIR_NIXPKG}/search_err"
}

bg_wait(){
  [[ -z "$BG_PID" ]] && { LOADING=0; return 0; }
...
# ─────────────────────────────────────────────────────────────
load_installed(){
  bg_start "Loading installed packages" "load"
...
do_search(){
  local q="$1"
  [[ -z "$q" ]] && return
  bg_start "Searching nixpkgs for '${q}'" "search"
...
do_install(){
  ...
  bg_start "Installing ${name}" "install"
...
main(){
  ...
  while true; do
    ...
    if [[ -n "$key" ]]; then
      ...
      tick=$(( tick + 1 ))
      # spinner + status update every ~100ms (2 ticks of 50ms)
      if (( tick % 2 == 0 )); then
        if [[ $LOADING -eq 1 ]]; then
          spinner_i=$(( (spinner_i + 1) % 10 ))
          local frame="${spinner_frames[$spinner_i]}"
          move $TERM_H 1
          printf "${C_HDR_BG}${C_YELLOW}${BOLD} %s %s ${RESET}${CLR_EOL}" \
            "$frame" "$LOADING_MSG"
        fi
      fi
      if (( tick % 20 == 0 )); then
