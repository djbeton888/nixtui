...
finish_search(){
  ...
  # Show any errors in log
  if [[ -f "$err_file" && -s "$err_file" ]]; then
    local err
    err=$(head -5 "$err_file")
    if echo "$err" | grep -qi "experimental"; then
      log_op "✗ nix search needs experimental features enabled"
      log_op "  Add to /etc/nix/nix.conf:"
      log_op "  experimental-features = nix-command flakes"
      log_op "  Then: sudo systemctl restart nix-daemon"
    else
      while IFS= read -r l; do
        [[ -n "$l" ]] && log_op "  $l"
      done < "$err_file"
    fi
    STATUS_MSG="Search failed — see log"
    STATUS_COLOR="$C_RED"
    return
  fi
