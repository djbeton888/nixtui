...
list_installed_with_version(){
  case "$MODE" in
    flakes|configuration)
      # Parse packages declared in nix config files
      local pkgs_file
      pkgs_file=$(find_packages_file 2>/dev/null || true)

      if [[ -n "$pkgs_file" ]]; then
        # Extract pkgs.xxx entries from the config
        grep -oP '\bpkgs\.\K[a-zA-Z0-9_\-]+' "$pkgs_file" 2>/dev/null \
          | sort -u \
          | while read -r name; do
              printf '%s\t\t(declared in config)\n' "$name"
            done
      fi

      # Also show any imperatively installed packages
      local json
...
load_installed(){
  ...
  (
    list_installed_with_version 2>/dev/null > "$RESULT_FILE" || true
  ) &
  BG_PID=$!
}
...
finish_load_installed(){
  ...
  if [[ ${#INST_LIST[@]} -eq 0 ]]; then
    log_op "⚠ No packages found in config or nix-env"
    log_op "  Config: $(find_packages_file 2>/dev/null || echo 'not found')"
    STATUS_MSG="No packages found — check config path"
