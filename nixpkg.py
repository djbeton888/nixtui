...
def init_colors():
    ...
    curses.init_pair(6, 8,                   -1)   # muted (dark gray)
    curses.init_pair(7, curses.COLOR_WHITE,  -1)   # white
    curses.init_pair(8, curses.COLOR_WHITE,  curses.COLOR_BLUE)   # selected
    curses.init_pair(9, curses.COLOR_WHITE,  236 if curses.COLORS >= 256 else -1)  # header bg
...
def draw_box(win, y, x, h, w, title: str, active: bool):
    ...
    color = c('blue') | attr if active else c('muted')
    try:
        win.attron(c('muted'))
        win.border()
        win.attroff(c('muted'))
    ...
    try:
        ...
        win.vline(1,   0, curses.ACS_VLINE, h-2)
        win.vline(1, w-1, curses.ACS_VLINE, h-2)
    ...
    label = f" {title} "
    try:
        win.addstr(0, 2, label, color | curses.A_BOLD)
    except:
        pass
...
def draw_panel_search(win, state: AppState, active: bool):
    ...
    if state.typing:
        ...
        q_display = f"/ {state.query}"
        try: win.addstr(1, 2, trunc(q_display, w-4), c('muted'))
        except: pass
...
def draw_topbar(win, state: AppState):
    ...
    try:
        win.addstr(0, 0, trunc(label, w), c('cyan') | curses.A_BOLD)
    except:
        pass
    win.noutrefresh()
...
def draw_bottombar(win, state: AppState):
    ...
    hint = "[tab]panel [j/k]↑↓ [/]search [i]install [d]remove [r]reload [u]rebuild index [q]quit"
    try: win.addstr(0, hint_start, trunc(hint, w - hint_start - 1), c('muted'))
    except: pass
    win.noutrefresh()
...
def preflight():
    ...
    if not has:
        print("\n  nixpkg — optional setup")
        print("  nix experimental-features needed for 'nix search' (not required for local index)")
        ans = input("  Add 'experimental-features = nix-command flakes' to /etc/nix/nix.conf? [y/N] ")
        if ans.strip().lower() == 'y':
            try:
                ...
                print("  ✓ Written")
                ans2 = input("  Restart nix-daemon? [y/N] ")
                if ans2.strip().lower() == 'y':
                    subprocess.run(['sudo', 'systemctl', 'restart', 'nix-daemon'])
                    print("  ✓ Restarted")
            except Exception as e:
                print(f"  Warning: {e}")
